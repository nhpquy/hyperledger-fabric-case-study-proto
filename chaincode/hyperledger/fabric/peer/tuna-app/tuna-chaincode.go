...
type Parkingspot struct {
	...
	ParkingTimes []ParkingTime `json:"parkingTimes"`
}
...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "searchTuna" {
		return s.searchTuna(APIstub, args)
...
func (s *SmartContract) queryTuna(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	fmt.Printf("- SVENZIK queryTuna:%s", args[0])
...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
	tuna := []Tuna{
		Tuna{Vessel: "SVEN", Location: "67.0006, -70.5476", Timestamp: "1504054225", Holder: "SVEN-EXT"},
		...
		Tuna{Vessel: "SVEN", Location: "51.9435, 8.2735", Timestamp: "1487745091", Holder: "Fatima",
			Parkingspot: Parkingspot{Name: "Tartu-Sobra-tee-1-315",
				ParkingTimes: []ParkingTime{
					ParkingTime{ParkingStart: time.Now(), ParkingEnd: time.Now(), CostPerMinute: 10},
				},
			},
		},
	}
...
func (s *SmartContract) recordTuna(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	json.Unmarshal([]byte(args[5]), &parkingSpot)

	var tuna = Tuna{Vessel: args[1], Location: args[2], Timestamp: args[3], Holder: args[4], Parkingspot: parkingSpot}
